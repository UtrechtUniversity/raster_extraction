...
def process_raster(args):
    """Modified worker function to process a single raster file in chunks."""
    df, raster_folder, raster_file = args
    ...
    values = np.full(df.shape[0], np.nan, dtype='float32')  
    ...
    for y_offset in range(0, dataset.RasterYSize, chunk_height):
        ...
        x_indices = np.floor((df['X'] - geotransform[0]) / geotransform[1]).astype(int)
        y_indices = np.floor((df['Y'] - geotransform[3]) / geotransform[5]).astype(int) - y_offset
...
def extract_values(input_csv, raster_folder, output_csv, in_crs, raster_crs, writemethod='concat',sep= ';',decimal='.'):
    ...
    with Pool(processes=cpu_count()) as pool:
        for raster_file in raster_files:
            args = (df.copy(), raster_folder, raster_file)
            result = pool.apply_async(process_raster, args=(args,), callback=lambda result: _update_progress(result, progress_counter, total_rasters))
    ...
    #Writing to CSV row by row without concatenating dataframes. This is memory efficient (no in-memory concatenation), but slower.
        ...
        assert len(df) == len(new_columns_df), "DataFrames must have the same number of rows" # Ensure both DataFrames have the same number of rows
